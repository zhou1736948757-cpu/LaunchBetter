...
## Performance Results(实测)

- **触点帧开销(§6 要求单独测量, GESTURE_DEBUG)**: 触点帧率 9-123 帧/秒;
  单帧分析耗时 **0.7~6.0 微秒**(微秒级可忽略)
- 空闲时(无触摸)零回调零开销, CPU ≈ 0(§6 目标达成)
- 回调只在事件时跳主线程(§88); 捏合为离散事件不走 120Hz display link(§90);
  分析就地执行(单锁区间, 用回调时间戳替代 Date(), 无每帧线程跳转)
- 优化点落地: receive() 单次加锁; 分析器状态锁保护(多设备多线程安全);
  权限误报修复(设备枚举>0 即授权生效, 无回调 ≠ 无权限)
