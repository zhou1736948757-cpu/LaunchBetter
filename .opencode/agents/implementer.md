...
## 命令纪律(重要)
- bash 每次只执行**一条简单命令**;禁止用 `&&`、`;`、`|`、`2>/dev/null` 组合多条命令
  (权限白名单只匹配单条命令, 组合会被自动拒绝导致卡死)
- 读文件用 read 工具(不要用 cat/ls);查目录用 ls 单条命令
- 只使用已授权命令: swift test / xcodebuild / python3 / mkdir / ls / cat / git status|diff|log
